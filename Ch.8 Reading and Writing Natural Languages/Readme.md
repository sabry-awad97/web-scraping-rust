...
fn retrieve_random_word(word_list: &HashMap<String, u32>) -> &str {
    ...
    let rand_index = rng.gen_range(1..=word_list_sum(word_list));
...
fn generate_markov_chain(
    ...
    for _ in 0..length {
        ...
        let next_word = retrieve_random_word(word_dict.get(&prev_word).unwrap_or(&default_hash));
...
async fn main() -> Result<(), Error> {
    ...
    let text = fetch_text(text_url).await?;
    let word_dict = build_word_dict(&text);

    let length = 100;
    let markov_chain = generate_markov_chain(&word_dict, length);
